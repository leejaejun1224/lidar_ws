...
public:
    ImageProjection():
        ...
        pubBevImage = nh.advertise<sensor_msgs::Image>("/bev_image", 1);
    ...
    void resetParameters(){
        ...
        bevImage = cv::Mat(720,1280,CV_8UC1, cv::Scalar::all(0));
        labelCount = 1;
    ...
    void projectPointCloud(){
        ...
        for (size_t i = 0; i < cloudSize; ++i){
            ...
            if ((atan2(thisPoint.x, thisPoint.y)*180/M_PI >60) && (thisPoint.z < -0.55) && (thisPoint.x < 10) && (atan2(thisPoint.x, thisPoint.y)*180/M_PI < 120)){
                fovCloud->points.push_back(thisPoint);
            }
            
            if (thisPoint.z < 1.5 && thisPoint.z > -0.5 && thisPoint.x*100 >= MIN_X && thisPoint.x*100 <= MAX_X && thisPoint.y*100 >= MIN_Y && thisPoint.y*100 <= MAX_Y) {
                int px = HEIGHT - (thisPoint.x*100 - MIN_X) / PIXEL_SIZE;
                int py = WIDTH - (thisPoint.y*100 - MIN_Y) / PIXEL_SIZE;

                if (px >= 0 && px < HEIGHT && py >= 0 && py < WIDTH) {
                    bevImage.at<uchar>(px, py) = 255;  // Set pixel to white
                }
            }
    ...
    void bevimage(){
        ...
        cv_img.image = bevImage;
        BevImage = cv_img.toImageMsg();
    }
    ...
    void clustering(){
        ...
        if (downSampledCloud->points.size() > 0){
            ...
            for(pcl::PointIndices getIndices: clusterIndices)
            {
                ...
                for(int index : getIndices.indices){ // ì—¬ê¸°ì„œ ClusterCloudì— ì¶”ê°€ì‹œí‚¬ ì¸ë±ìŠ¤ë¥¼ ê±¸ëŸ¬ë³´ìž ì—¬ê¸°ì— í•´ë‹¹í•˜ëŠ” ì¸ë±ìŠ¤ë§Œ ê±°ë¥¼ê±°ìž–ì•„. 
                    pcl::PointXYZI pt = downSampledCloud->points[index];
                    pt.intensity = (float)(j + 1);

                    if(pt.y > -10.0 && pt.y < 10.0){ // ì´ ë¶€ë¶„ì´ ë„ë¡œì˜ ì˜ì—­ì— í•´ë‹¹í•˜ë©´ ì´ ClusterCloudì— ì¶”ê°€ë¥¼ ì‹œì¼œ ì£¼ê² ë‹¤ëŠ” ë‚´ìš©ìž„
...
void filteringCone(){
    ...
    for (pcl::PointCloud<pcl::PointXYZI>::Ptr cluster : ClusterClouds){
        ...
        if( (maxPoint.z < 0.0) && (maxPoint.y < -0.8 && minPoint.y > -3.0) && (minPoint.x > -1.0 && maxPoint.x < 5.0) && (maxPoint.x-minPoint.x < 0.6 && maxPoint.y-minPoint.y < 0.6)){
